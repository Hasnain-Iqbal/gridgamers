  ...
  const filteredCareers = careers.filter(
    (career) =>
      ...
      (selectedLocation === "View all" ||
        career.location === selectedLocation) &&
      (selectedEmploymentType === "View all" ||
        career.type === selectedEmploymentType)
  );

  return (
    <div className="p-8 pb-16 text-white-500 bg-[#141212]  min-h-screen">
      <h1 className=" py-[3.75rem]"></h1>
      {/* Decrease gap for phones */}
      <div
        className="w-[55%] h-[125vh] mx-auto flex items-center justify-center"
        style={{ fontFamily: "Montserrat" }}
      >
        <div className="text-center text-[6vw] font-[600] flex-col space-y-16">
          <p>Career at GRID</p>
          <img
      ...
      {/* Department Filters */}
      <section className="max-w-6xl mx-auto flex flex-wrap gap-4 justify-center mb-8">
        {departments.map((department, index) => (
          <button
            key={index}
            className={`py-2 px-4 rounded-full  ${
              selectedDepartment === department
                ? "bg-primary text-black-900"
                : "bg-[#43424d] text-white-500"
            }`}
      ...
      {/* Darker color: #02070c */}
      {/* Location and Employment Type Filter Dropdowns */}
      <section className="max-w-6xl mx-auto flex flex-wrap gap-12 justify-evenly items-center mb-12">
      ...
      {/* Lighter colorer for div: #17171a */}
      <section className="max-w-6xl mx-auto px-6 rounded-md shadow-md">
        <ul>
          {filteredCareers.map((career, index) => (
            <li
              key={index}
              className={`flex justify-between py-5  ${
                index !== 0 ? "border-t border-gray-500 mt-2" : ""
              }`}
            >
              <div>
                <h3 className="text-3xl font-semibold text-[#433bff] mb-1">
                  {career.title}
                </h3>
                <p className="text-sm text-gray-500 mb-3">
                  {career.department}
                </p>
                <p className="text-md text-white-500 mb-2">
                  {career.location === "Remote" ? "" : career.location}
                </p>
                ...
                <div className="flex">
                  {career.location === "Remote" && (
                    <button className="bg-[#43424d] text-white-500 my-auto mr-2 px-4 py-1 rounded-full flex items-center pointer-events-none">
