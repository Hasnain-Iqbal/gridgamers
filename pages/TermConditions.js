export default function Home({ TermConditions }) {
  return (
    <div className="xs:px-0 md:px-20 3xl:px-0  xs:max-w-screen-xs sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg  xl:max-w-screen-xl  2xl:max-w-screen-2xl   py-0 md:py-16    mt-[40px] lg:mt-24 mx-auto">
      ...
      <div
        ...
        dangerouslySetInnerHTML={{ __html: TermConditions }}
      />
    </div>
  );
}
export async function getStaticProps() {
  ...
  const response = await fetch(
    "https://hubapi.gridgamers.com/api/v1/setting/privacypolicy-termscondition?SubCatagoryId=13"
  );
  ...
  return {
    props: { TermConditions: data.response.subCategoryDescription },
  };
}
